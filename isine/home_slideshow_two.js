...
router.get('/', cek_login, function(req, res) {
  connection.query("SELECT * from management_home where slideshow=2 AND deleted=0", function(err, rows, fields) {
          if (err) throw err;
          numRows = rows.length;
          console.log(rows);
           res.render('content-backoffice/home_slideshow_two/list',{data : rows});
        })
});

router.get('/insert', cek_login, function(req, res) {

  res.render('content-backoffice/home_slideshow_two/insert');
});
...
router.get('/edit/:id', cek_login, function(req, res) {
 connection.query("SELECT * from management_home where id='"+req.params.id+"'", function(err, rows, fields) {
          if (err) throw err;
         // jmbt.concat(rows);
          res.render('content-backoffice/home_slideshow_two/edit',{data : rows});
        })
...
router.get('/delete/:id', cek_login, function(req, res) {
  ...
  res.redirect('/home_slideshow_two');
});
...
router.post('/submit_insert', upload.single('foto'), function(req, res){
 var oke = "";
  ...
  connection.query("insert into management_home (foto, slideshow) VALUES ('"+oke+"', '2')", function(err, rows, fields) {
    if (err) throw err;
    numRows = rows.affectedRows;
  })
  res.redirect('/home_slideshow_two');
