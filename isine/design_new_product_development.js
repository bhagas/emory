...
connection.query("SELECT * from design where id='1'", function(err, rows, fields) {
          if (err) throw err;
         // jmbt.concat(rows);
          res.render('content-backoffice/design_new_product_development/edit',{data : rows});
        })
  });
...
router.post('/submit_edit', upload.fields([{ name: 'npd_1', maxCount: 1 }, { name: 'npd_2', maxCount: 1 }]),  function(req, res){
  var oke = "";
  if(req.files['npd_1']){
     oke = "npd_1='"+req.files['npd_1'][0].filename+"'";
  }
  var okee = "";
  if(req.files['npd_2']){
     okee = "npd_2='"+req.files['npd_2'][0].filename+"'";
  }
  var koma="";
  if(oke!="" && okee!=""){
    koma=",";
  ...
  res.redirect('/design_new_product_development/edit');
})
