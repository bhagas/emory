...
// });
router.get('/get_dataa', function(req, res) {
  connection.query("SELECT * from contact where id='1'", function(err, rows, fields) {
          if (err) throw err;
         // jmbt.concat(rows);
...
router.get('/edit', cek_login, function(req, res) {
  connection.query("SELECT * from contact where id='1'", function(err, rows, fields) {
          if (err) throw err;
         // jmbt.concat(rows);
          res.render('content-backoffice/contact/edit',{data : rows});
        })
});
...
//       return;
//     } else {
//         connection.query("insert kritik_saran user (username, pwd, fullname, NIP, email, telp) VALUES ('"+req.body.username+"', '"+sha1(req.body.pwd)+"', '"+req.body.fullname+"', '"+req.body.NIP+"', '"+req.body.email+"', '"+req.body.telp+"')", function(err, rows, fields) {
//         if (err) throw err;
//         numRows = rows.affectedRows;
//       })
...
router.post('/submit_edit', upload.array(),  function(req, res){
  ...
  // senjata
  //console.log(req.body)
 connection.query("update contact SET address='"+req.body.address+"', phone='"+req.body.phone+"', fax='"+req.body.fax+"', info='"+req.body.info+"', marketing='"+req.body.marketing+"' WHERE id='"+req.body.id+"' ", function(err, rows, fields) {
    if (err) throw err;
    numRows = rows.affectedRows;
  })
