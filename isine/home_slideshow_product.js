...
connection.query("SELECT * from management_home where slideshow=3 AND deleted=0", function(err, rows, fields) {
          if (err) throw err;
          numRows = rows.length;
          console.log(rows);
           res.render('content-backoffice/home_slideshow_product/list',{data : rows});
        })
});

router.get('/insert', cek_login, function(req, res) {

  res.render('content-backoffice/home_slideshow_product/insert');
});
...
connection.query("SELECT * from management_home where id='"+req.params.id+"'", function(err, rows, fields) {
          if (err) throw err;
         // jmbt.concat(rows);
          res.render('content-backoffice/home_slideshow_product/edit',{data : rows});
        })
});
...
router.get('/delete/:id', cek_login, function(req, res) {
  ...
  res.redirect('/home_slideshow_product');
});
...
router.post('/submit_insert', upload.single('foto'), function(req, res){
  ...
  connection.query("insert into management_home (foto, slideshow) VALUES ('"+oke+"', '3')", function(err, rows, fields) {
    if (err) throw err;
    numRows = rows.affectedRows;
  })
  res.redirect('/home_slideshow_product');
