...
connection.query("SELECT * from design where id='1'", function(err, rows, fields) {
          if (err) throw err;
         // jmbt.concat(rows);
          res.render('content-backoffice/design_manufacturing/edit',{data : rows});
        })
});
...
router.post('/submit_edit', upload.fields([{ name: 'manufacturing_1', maxCount: 1 }, { name: 'manufacturing_2', maxCount: 1 }]),  function(req, res){
  var oke = "";
  if(req.files['manufacturing_1']){
     oke = "manufacturing_1='"+req.files['manufacturing_1'][0].filename+"'";
  }
  var okee = "";
  if(req.files['manufacturing_2']){
     okee = "manufacturing_2='"+req.files['manufacturing_2'][0].filename+"'";
  }
  var koma="";
  if(oke!="" && okee!=""){
    koma=",";
  ...
  // req.body.nameopo
  // senjata
  //console.log(req.files['manufacturing_1'][0].filename)
  connection.query("update design SET "+oke+" "+koma+" "+okee+" WHERE id='"+req.body.id+"' ", function(err, rows, fields) {
    if (err) throw err;
    numRows = rows.affectedRows;
  })
  res.redirect('/design_manufacturing/edit');
})
