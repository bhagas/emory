...
router.get('/edit', cek_login, function(req, res) {
  connection.query("SELECT * from about_us where id='1'", function(err, rows, fields) {
          if (err) throw err;
         // jmbt.concat(rows);
          res.render('content-backoffice/about_us_mission/edit',{data : rows});
        })
...
router.post('/submit_edit', upload.array(),  function(req, res){
  ...
  // senjata
  //console.log(req.body)
  connection.query("update about_us SET mission='"+req.body.mission+"' WHERE id='"+req.body.id+"' ", function(err, rows, fields) {
    if (err) throw err;
    numRows = rows.affectedRows;
  })
  res.redirect('/about_us_mission/edit');
})
